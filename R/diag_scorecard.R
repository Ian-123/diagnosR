...
#' @keywords internal
#' @noRd
diag_multicollinearity_tests <- function(model, vif_threshold = 10, kappa_threshold = 30) {
...
diag_scorecard <- function(model,
  ...
  # VIF table
  if (!is.null(mc) && length(mc$vif_vector) > 0) {
    if (any(is.finite(mc$vif_vector))) {
      cat("\nVIF (per term)\n")
      print(sort(mc$vif_vector, decreasing = TRUE))
    } else {
      cat("\nVIF not available (all NA/Inf after preprocessing).\n")
    }
  }
